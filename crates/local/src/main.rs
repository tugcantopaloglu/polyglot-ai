...
async fn run_tui(tool_manager: LocalToolManager, config: &LocalConfig, mut history_manager: HistoryManager) -> Result<()> {
    ...
    tokio::spawn(async {
        if let Some(msg) = polyglot_common::check_updates_on_startup("polyglot-local").await {
...
async fn run_update(check_only: bool, force: bool) -> Result<()> {
    ...
    let update_info = polyglot_common::check_for_updates_github("polyglot-local").await?;
